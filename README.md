...
* **Biometric Authentication:** Real-time fingerprint matching using the Adafruit Fingerprint library.
...
* **Wi-Fi Management & Auto-Failover:** Dynamic connection manager that scans local networks, saves multiple Wi-Fi credentials to SPIFFS, and automatically connects to the strongest available access point.
* **OLED & LED Status Ring:** Displays real-time status messages, system IP address, clock time via NTP, and animated ring lighting feedback during operations.
* **Remote Device Testing:** Web UI controls for testing fingerprint optics, OLED pixel diagnostic fills, and LED ring colors.
* **mDNS Support:** Easy access via local domain `http://esp32-fp.local.
...
* Optical/Capacitive Fingerprint Sensor with Ring LED
...
| **Fingerprint RX** | GPIO 4 | Serial2 RX |
| **Fingerprint TX** | GPIO 5 | Serial2 TX |
...
* **Key Libraries:** `WiFi`, `WebServer`, `SPIFFS`, `ESPmDNS`, `ArduinoJson`, `Adafruit_Fingerprint`, `U8g2lib`
* **Frontend:** HTML5, CSS3, JavaScript (ES6+), S
